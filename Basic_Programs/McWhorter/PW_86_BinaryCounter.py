...
press =0
tUp=time.ticks_ms() ## tUp is when the button is pushed and reads 1
tDown=time.ticks_ms()
butStateOld=0
x=0
def ButPress(pin):
    global press,tUp,tDown,butStateOld
    ...
    if butState==0:
        tDown =time.ticks_ms()
    if butState==1 and butStateOld==0 and (tUp-tDown>25):
        print('Triggered: ',press)
        rLed.toggle()
    butStateOld=butState
Button.irq(trigger=Pin.IRQ_FALLING|Pin.IRQ_RISING,handler=ButPress)
try:
    while True:
