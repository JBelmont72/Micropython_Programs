...
PW 106
add sectrets
pico program at bottom

The Pico is configures as the server, and  desktop pc or laptop is configures to be the client. Will be running python on  PC. The project requests the user on the PC to specify a desired color. The color is then sent to the Pico, the Server. Then the Pico sets that color to /‘ON/’. The pi pico is powered by a breadboard power bank, and there is no need for any connections to the pico.
'''
import network
